...
#undef c2l //вызывается второй по счету
#define c2l(c,l) \
( l = ((unsigned long)(*((c)++))), \
  l |= (((unsigned long)(*((c)++)))<<8), \
...
#undef p_c2l_p //  вызывается третьим по счету
...
// наш третий этап - инициализация буфера
...
void MD5_Init(MD5_CTX *c) // от основной функции
